...
names = ['None', 'Dastan', 'Zhanat', 'Rustam', 'Zhanik', 'Merey', 'Bauka', 'Serzhan'] 
...
while True:
    ...
    while TIMER >= 0:
        ...
        if (TIMER >= 10):
            ...
            timer = "00:0"+str(TIMER)
        cv2.putText(frame, timer, 
                    (225,90), cv2.FONT_HERSHEY_PLAIN,
                    4, (0, 255, 255),
                    3, cv2.LINE_AA)
        cv2.imshow("Face Landmarks", frame)
        cv2.waitKey(125)        
    ...
    if (obj1.score > obj2.score):
        cv2.putText(frame, "Player 1 won", (170, 240), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255), 3)
    elif (obj1.score < obj2.score):
        cv2.putText(frame, "Player 2 won", (170, 240), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255), 3)
    else:
        cv2.putText(frame, "Draw", (250, 240), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255), 3)
    cTime = time.time()
    fps = 1/(cTime-pTime)
    pTime = cTime        
    cv2.putText(frame,f'FPS:{int(fps)}', (280,30), cv2.FONT_HERSHEY_PLAIN, 2, (255,0,0), 2)
